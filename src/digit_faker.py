...
class DigitFaker:
    ...
    def __init__(
        self,
        digit_detection_method="easyocr",
        texture_match="sam",
        blending="gaussian",
        debug=False,
    ):
    ...
    def find_all_possible_changes(self, digit, dash_count_max=2):
        super_set_digits = [
            d
            for d in self.DIGIT_CONTAIN_DASH_INDEX
            if self.DIGIT_CONTAIN_DASH_INDEX[d].issuperset(
                self.DIGIT_CONTAIN_DASH_INDEX[digit]
            )
            and d != digit
        ]
        ...
        if self.digit_detection_method == "easyocr":
            ...
            if self.debug:
                ...
                for contour in contours:
                    ...
                    if 5 < w and 5 < h:
                        cv2.rectangle(
                            contour_image, (x, y), (x + w, y + h), (0, 255, 0), 2
                        )
                cv2.imshow("contour image", contour_image)
                cv2.waitKey(0)

            contours = self.filter_opencv_contours(contours, image)
            self.wrapped_image, self.inv_transform_matrix = (
                self.converting_contours_to_digits(contours, image)
            )
    ...
    def interactive_clock_modification(self):
        ...
        while True:
            if step == 1:
                ...
                print(
                    "Enter a number (1 to {}) or 'q' to quit:".format(len(self.digits))
                )
                ...
                print(
                    "Do you want to 'add' or 'remove' dashes? (Type 'back' to go back)"
                )
                ...
                if action == "add":
                    ...
                    for new_digit in addition_options:
                        print(
                            f"Add dashes {addition_options[new_digit]} to get {new_digit}"
                        )
                else:
                    ...
                    for new_digit in removal_options:
                        print(
                            f"Remove dashes {removal_options[new_digit]} to get {new_digit}"
                        )

                print(
                    "Enter the digit you want to change to, or type 'back' to go back:"
                )
                user_input = input("> ").strip().lower()
                ...
                if user_input == "back":
                    ...
                    if action == "add" and new_digit in addition_options:
                        modified_image, pad_offset_mat = self.add_dashes(
                            self.wrapped_image, digit_index, addition_options[new_digit]
                        )
                        full_modified_image = self.inv_to_original_image(
                            self.image,
                            modified_image,
                        ...
                        modified_image = self.hide_dashes(
                            self.wrapped_image, digit_index, removal_options[new_digit]
                        )
                        full_modified_image = self.inv_to_original_image(
                            self.image, modified_image, self.inv_transform_matrix
                        )
                        cv2.imshow("original image", self.image)
                        cv2.imshow("full modified image", full_modified_image)
                        cv2.waitKey(0)
        ...
        for sub_digit in self.DIGIT_CONTAIN_DASH_INDEX:
            if sub_digit != digit:
                if self.DIGIT_CONTAIN_DASH_INDEX[sub_digit].issubset(dashes):
                    possible_removals[sub_digit] = dashes.difference(
                        self.DIGIT_CONTAIN_DASH_INDEX[sub_digit]
                    )
        ...
        for sup_digit in self.DIGIT_CONTAIN_DASH_INDEX:
            if sup_digit != digit:
                if self.DIGIT_CONTAIN_DASH_INDEX[sup_digit].issuperset(dashes):
                    needed_dashes = self.DIGIT_CONTAIN_DASH_INDEX[sup_digit].difference(
                        dashes
                    )
    ...
    def filter_opencv_contours(self, contours, image):
        ...
        for contour in contours:
            ...
            if (
                cv2.contourArea(cv2.convexHull(contour)) == 0
                or cv2.contourArea(contour) / cv2.contourArea(cv2.convexHull(contour))
                < 0.8
                or cv2.contourArea(contour) < 100
            ):
                continue
            # finally, aspect ratio check

            if (
                w >= h
                and (w < min_rect_width or h / w < min_aspect_ratio)
                or h >= w
                and (h < min_rect_height or w / h < min_aspect_ratio)
                or w > max_rect_width
                or h > max_rect_height
            ):
                continue
            # next, ensure that the most pixels in the contour are bright, threshold: 50
            mask = np.zeros_like(image)
            cv2.drawContours(mask, [contour], -1, (255, 255, 255), thickness=cv2.FILLED)
            contour_pixels = image[mask == 255]
            bright_pixel_count = np.sum(contour_pixels > 50)
            total_pixel_count = contour_pixels.size
            bright_ratio = bright_pixel_count / total_pixel_count
            if bright_ratio < 0.8:
                continue

            if x <= 0 or y <= 0 or x + w >= full_width or y + h >= full_height:
                continue
            else:
    ...
    def converting_contours_to_digits(self, contours, image):
        ...
        all_big_x_width = [
            ...
            and compute_contour_aspect_ratio(contour) < 0.5
        ]
        self.avg_big_x_width = sum(all_big_x_width) // len(all_big_x_width)
        all_small_x_width = [
            cv2.boundingRect(contour)[2]
            for contour in contours
            if cv2.boundingRect(contour)[3] > cv2.boundingRect(contour)[2]
            and compute_contour_aspect_ratio(contour) < 0.5
        ]
        ...
        for i in range(1, len(contours)):
            ...
            r2 = x2 + w2
            if (
                self.avg_big_x_width * 0.8 < x2 - x1 < self.avg_big_x_width * 1.2
                and r2 - r1 <= self.avg_big_x_width * 1.2
            ):
                on_right = True

            if (
                x2 < r1
                or x2 - x1 <= self.avg_small_x_width * 0.5
                or self.avg_big_x_width * 0.8 < x2 - x1 < self.avg_big_x_width * 1.2
                and r2 - r1 <= self.avg_big_x_width * 1.2
                and not on_right
            ):
        ...
        warpped_image, new_digit_contours, inv_transform_matrix = (
            self.perspective_transform(image, digit_contours)
        )
        ...
        flattened_contours = [
            contour for digit_contour in new_digit_contours for contour in digit_contour
        ]
        ...
        for i, digit_contour in enumerate(new_digit_contours):
            recognized_digit, dashes_dict = self.compute_digit_from_contours(
                digit_contour, warpped_image
            )
            self.digits.append(recognized_digit)
            self.dashes_dicts.append(dashes_dict)
        print(
            f"Recognized time: {self.digits[0]}{self.digits[1]}:{self.digits[2]}{self.digits[3]}"
        )
    ...
    def find_four_corners_old(self, four_contours, margin=0):
        left_contours, right_contours = four_contours[0], four_contours[-1]
        left_most = (
            min([cv2.boundingRect(contour)[0] for contour in left_contours]) - margin
        )

        left_top = (
            min([cv2.boundingRect(contour)[1] for contour in left_contours]) - margin
        )
        left_bottom = (
            max(
                [
        ...
        right_top = (
            min([cv2.boundingRect(contour)[1] for contour in right_contours]) - margin
        )
        right_bottom = (
            max(
                [
        ...
        return [
            (left_most, left_top),
            (right_most, right_top),
            (right_most, right_bottom),
            (left_most, left_bottom),
        ]

    def find_four_corners(self, four_contours, margin=0):
        def order_points_clockwise(points):
            """
            Order the four points in clockwise order: top-left, top-right, bottom-right, bottom-left.

            :param points: Array of four points from cv2.boxPoints()
            :return: Sorted points in clockwise order
            """
            # Step 1: Find the sum and difference of the points
        ...
        left_rects = [cv2.minAreaRect(contour) for contour in left_contours]
        left_rects_sorted = sorted(left_rects, key=lambda x: x[0][1])
        left_rect_contours = [
            order_points_clockwise(np.intp(cv2.boxPoints(rect)))
            for rect in left_rects_sorted
        ]
        top_left = left_rect_contours[0][0]
        bottom_left = left_rect_contours[-1][3]

        right_rects = [cv2.minAreaRect(contour) for contour in right_contours]
        right_rects_sorted = sorted(right_rects, key=lambda x: x[0][1])
        right_rect_contours = [
            order_points_clockwise(np.intp(cv2.boxPoints(rect)))
            for rect in right_rects_sorted
        ]
        print("right_rect_contours: ", right_rect_contours)
        top_right = right_rect_contours[0][1]
        bottom_right = right_rect_contours[-1][2]
        print("bottom_right: ", bottom_right)
        return [top_left, top_right, bottom_right, bottom_left]

    def find_side_most_pixel(self, contours, direction="up"):
        # find the top/bottom most pixel of all contours and use the y coord
        if direction == "up":
            extreme_y = float("inf")
        if direction == "down":
            extreme_y = float("-inf")
        for contour in contours:
            for point in contour:
                x, y = point[0]
                if direction == "up":
                    extreme_y = min(extreme_y, y)
                if direction == "down":
                    extreme_y = max(extreme_y, y)
        return (
            int(extreme_y)
            if extreme_y != float("inf") and extreme_y != float("-inf")
            else None
        )

    def perspective_transform(
        self, image, four_contours, output_width=1300, output_height=320
    ):
        # Define the points for the output perspective transform

        corners = self.find_four_corners(four_contours, margin=5)
        ...
        new_four_contours = [
            transform_contours(contours, M) for contours in four_contours
        ]
        return warped, new_four_contours, M_inv

    def inv_to_original_image(
        self, image, wrapped_image, inv_transform_matrix, pad_transform_matrix=None
    ):
        ...
        wrapped_back = cv2.warpPerspective(
            wrapped_image,
        ...
        mask = cv2.warpPerspective(
            white_image, inv_transform_matrix, (image.shape[1], image.shape[0])
        )
        ...
        modified_image = cv2.add(
            modified_image, cv2.bitwise_and(wrapped_back, wrapped_back, mask=mask)
        )
    ...
    def compute_digit_from_contours_old(self, contours, image):
        ...
        regular_contours = [
            contour
            for contour in contours
            if compute_contour_aspect_ratio(contour) < 0.5
        ]
        for contour in regular_contours:
            ...
            if w > h:  # 5, 6, or 7
                ...
                if abs(max_right - x) <= self.avg_big_x_width / 2:  # on the right
                    if (
                        abs(y - self.all_y_min) <= self.avg_big_y_height / 1.2
                    ):  # should be 2, but perspective transform should work
        ...
        for contour in bad_contours:
            ...
            if (
                abs((x + w) - max_right) >= self.avg_small_x_width / 4
            ):  # has to not include anything on the right
                if abs(y - self.all_y_min) <= self.avg_big_y_height / 2:
                    dashes_indexes.add(5)
                    dashes_indexes.add(1)
                else:
    ...
    def compute_digit_from_contours(self, contours, image):
        ...
        for contour in contours:
            ...
            if self.debug:
                # visualize the contour's convex hull
    ...
    def hide_dashes(self, image, digit_index, dash_indexes):
        ...
        for dash_index in dash_indexes:
            ...
            inpainted_image = self.inpaint_with_dark_colors(
                inpainted_image, gray_mask, 100
            )
    ...
    def add_dashes(
        ...
        pad_offset_mat = None
        if (
            digit_index == 0 and self.digits[0] == 1 and not smart_handle_first_1
        ):  # meaning we have to pad the image whatsoever
            # find the longest width in all dashes of all digits
            image_left_offset = max(
                [
                    cv2.boundingRect(contour)[2]
                    for digit_index in range(1, 4)
                    for contour in self.dashes_dicts[digit_index].values()
                ]
            )
            # pad the image to the left
            faked_image = cv2.copyMakeBorder(
                faked_image, 0, 0, image_left_offset, 0, cv2.BORDER_CONSTANT, value=0
            )
        ...
        for dash_index in dash_indexes:
            ...
            dash_closest_digit_index = sorted(
                self.available_dashes[dash_index], key=lambda x: abs(x - digit_index)
            )[0]
            contour = self.dashes_dicts[dash_closest_digit_index][dash_index]
            if apply_smoothing:
                epsilon = 0.005 * cv2.arcLength(
                    contour, True
                )  # Adjust epsilon for smoothness
                contour = cv2.approxPolyDP(contour, epsilon, True)
            # copy to the respective location
            destination_right_most_x = max(
                [
                    cv2.boundingRect(contour)[0] + cv2.boundingRect(contour)[2]
                    for contour in self.dashes_dicts[digit_index].values()
                ]
            )
            source_right_most_x = max(
                [
                    cv2.boundingRect(contour)[0] + cv2.boundingRect(contour)[2]
                    for contour in self.dashes_dicts[dash_closest_digit_index].values()
                ]
            )
            ...
            if image_left_offset != 0:  # mean a left-most one, need to offset these as
                mask = cv2.copyMakeBorder(
                    mask, 0, 0, image_left_offset, 0, cv2.BORDER_CONSTANT, value=0
                )
                old_region = cv2.copyMakeBorder(
                    old_region, 0, 0, image_left_offset, 0, cv2.BORDER_CONSTANT, value=0
                )

            translation_matrix = np.float32([[1, 0, x_offset], [0, 1, 0]])
            translated_region = cv2.warpAffine(
                old_region,
                translation_matrix,
                (faked_image.shape[1], faked_image.shape[0]),
            )
            translated_mask = cv2.warpAffine(
                mask, translation_matrix, (faked_image.shape[1], faked_image.shape[0])
            )
    ...
    def hide_dashes_static(self, image, digit_index, dash_indexes):
        ...
        for dash_index in dash_indexes:
            ...
            for y, x in contour_coords:
                ...
                left_region = filled_image[
                    max(0, y - 1) : min(y + 2, filled_image.shape[0]), max(0, x - 3) : x
                ]

                if left_region.size > 0:  # Ensure the region exists
                    # Find the darkest pixel in the left region
                    darkest_pixel = np.min(
                        left_region, axis=(0, 1)
                    )  # Min intensity for each channel (B, G, R)
                    filled_image[y, x] = (
                        darkest_pixel  # Replace current pixel with the darkest pixel
                    )
...
if __name__ == "__main__":
    ...
    # digit_faker.fake_digit("data/white_one.png")
